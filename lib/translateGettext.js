...
module.exports = (input, markAsFuzzy, overwrite, callback, output) => {
    ...
    if (!language) {
        console.error(`${input} does not contain any language definition in its header. To perform the translation, you need to specify a language.`)
        return
    }

    if (!google_translate.languages[language]) {
        language = language.replace('_', '-').toLowerCase()
    }

    if (!google_translate.languages[language] && language.includes('-')) {
